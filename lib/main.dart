...
class _MyAppState extends State<MyApp> {
  ...
  void _setFilters(Map<String,bool> fillterData){
    setState(() {
      _fillters = fillterData;

      _avalabileMeals = DUMMY_MEALS.where(
        (meal){
          if(_fillters['gluten'] && !meal.isGlutenFree){
            return false;
          }
          if(_fillters['loctose'] && !meal.isLactoseFree){
            return false;
          }
          if(_fillters['vegan'] && !meal.isVegan){
            return false;
          }
          if(_fillters['vegetarain'] && !meal.isVegetarian){
            return false;
          }
          return true;
        }
  ...
  Widget build(BuildContext context){
    return MaterialApp(
      ...
      routes: {
        ...
        MealDetailScreen.routeName : (ctx) => MealDetailScreen(_toggleFavorit,_isMealFavorite),
        FiltersScreen.routeName : (ctx) => FiltersScreen(_fillters,_setFilters),
      },
